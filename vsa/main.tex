...
A typical practical DSL may have up to $10^{30}$ programs consistent with a given ispec~\cite{vldb12:semantic}.
Any synthesis strategy for solving \Cref{problem:syn:general} requires a data structure for succinctly representing such a huge number of
...
In this section, we formalize the generic definition of VSA as an essential primitive for synthesis algorithms, expanding
upon specific applications that were explored previously by \citeauthor{vsa:Mitchell}, \citeauthor{lau:smartedit}, and us.
We define a set of efficient operations over this data structure that are used by several synthesis algorithms,
...
Hereinafter we use the words ``program set'' and ``version space algebra'' interchangeably, and use the same notation for both concepts.
...
We define three operations over VSAs, which are used during the synthesis process: \emph{intersection},
...
In this section, we define VSA intersection generically, and also introduce ranking and clustering of VSAs, which are our novel contributions.
...
Given a conjunctive ispec $\spec_1 \wedge \spec_2$, one possible synthesis approach is to learn a set of programs $\vsa_1$ consistent with
...
\begin{align*}
    ...
    \clustering[\vsaunion(\vsa_1,\text{...},\vsa_k)] &\bydef \mathsf{G}\bigl(\textstyle\bigcup\nolimits_k
...
\begin{defn}[Filtered VSA]
    Given a VSA $\vsa$ and an ispec $\spec$ on $N$, a \emph{filtered VSA} $\mathsf{Filter}(\vsa, \spec)$ is defined as
...
\begin{theorem}
    ...
    \begin{enumerate}[nosep,label=(\arabic*)]
        \item $\mathsf{Filter}(\vsa, \spec) = \vsaunion\bigl\{\vsa_j \bigmid \langle \values_j \mapsto \vsa_j \rangle
...
\end{theorem}
% \vspace{-\baselineskip}
% \noindent In~\Cref{sec:topdown}, we show another application of the clustering operation, where it helps us to partition the search space of
% candidate programs into subspaces with different behavior on a given input state.
% We can then parallelize the search process over such independent subspaces of programs.
...
However, in \Cref{sec:experiments} we show that these values usually remain small for any practical DSL.
